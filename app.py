...
# Paleta Institucional (Burdeo, Verde, Gris oscuro, Gris claro, Verde claro, Rojo tenue)
...
def cargar_datos(report_file, empleados_file):
    ...
    # Función de lectura segura multimodo (CSV y Excel)
    ...
    # Limpieza estandarizada de RUTs para asegurar un cruce perfecto
    ...
    # Motor de clasificación de estado vía observaciones
    ...
    # Cálculo seguro de horas efectivas
    ...
    # Normalización de formato de fecha para conteo exacto de días
...
if report_file and empleados_file:
    ...
    if 'Nombre Centro Costo 1' in df.columns:
        centros_costo = sorted(df['Nombre Centro Costo 1'].dropna().unique().tolist())
    ...
    # --- 5. AUDITORÍA DE DISPONIBILIDAD (GRÁFICOS) ---
    ...
    with colB:
        if 'Equipo' in df.columns and 'Horas_Efectivas' in df.columns:
            ...
            fig_horas = px.bar(horas_por_equipo, x='Horas_Efectivas', y='Equipo', orientation='h', 
                               ...
                               color_discrete_sequence=['#800020']) # Burdeo
    ...
    if 'Equipo' in df.columns and 'Fecha reporte' in df.columns:
        # Lógica: Contar fechas únicas agrupadas por Equipo
        ...
        df_inactivos = df[df['Horas_Efectivas'] == 0].groupby('Equipo')['Fecha reporte'].nunique().reset_index()
        df_inactivos.rename(columns={'Fecha reporte': 'Total Días Inactivos (0 Horas)'}, inplace=True)
        
        # Desglose de motivos pivotando la tabla
        df_motivos = df[df['Horas_Efectivas'] == 0].groupby(['Equipo', 'Estado_Equipo'])['Fecha reporte'].nunique().unstack(fill_value=0).reset_index()
        
        # Unir métricas
        resumen_facturacion = pd.merge(df_trabajados, df_inactivos, on='Equipo', how='outer').fillna(0)
        if not df_motivos.empty:
            resumen_facturacion = pd.merge(resumen_facturacion, df_motivos, on='Equipo', how='outer').fillna(0)
            
        # Formateo visual numérico
        for col in resumen_facturacion.columns:
            if col != 'Equipo':
                resumen_facturacion[col] = resumen_facturacion[col].astype(int)
                
        # Sumatoria final del mes
        resumen_facturacion.insert(1, 'Total Días Auditados', resumen_facturacion['Días Trabajados (Con Horas)'] + resumen_facturacion['Total Días Inactivos (0 Horas)'])
        ...
        if not df_cd.empty:
            ...
            def clasificar_anomalia(row):
                ...
                if row['Cantidad (Ingreso Combustible)'] > 0 and row['KM_Recorridos'] > 0:
                    if row['Rendimiento (KM/L)'] < 4: anomalias.append("Alerta Fuga: Rendimiento crítico (< 4 KM/L)")
                    elif row['Rendimiento (KM/L)'] > 16: anomalias.append("Error Registro: Rendimiento irreal (> 16 KM/L)")
            ...
            # Filtrar e imprimir inconsistencias
            ...
            # Gráfico en Verde Institucional
    ...
    columnas_vista = ['Fecha reporte', 'Equipo', 'Estado_Equipo', 'RUT_clean', 'Nombre', 'Cargo', 'Horas_Efectivas', 'Observaciones']
    if 'Nombre Centro Costo 1' in df.columns:
        columnas_vista.insert(7, 'Nombre Centro Costo 1')
        
    columnas_existentes = [col for col in columnas_vista if col in df.columns]
    
    if 'Equipo' in df.columns:
        st.dataframe(df[columnas_existentes].sort_values(by=['Equipo', 'Fecha reporte']), use_container_width=True)
    else:
